...
planner_model.ainvoke = asyncio.coroutine(lambda *a, **kw: MagicMock(content='{"strategy": "temporal"}'))
...
fc_verify.get_llm = lambda: MagicMock(ainvoke=asyncio.coroutine(lambda *a, **kw: MagicMock(content='{"verified_claims": [{"id": "c0", "text": "JWST launched in December 2021.", "verdict": "SUPPORTED", "confidence": 0.95, "sources": ["https://nasa.example/jwst"]}], "follow_up_queries": []}')))
