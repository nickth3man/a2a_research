"""Ranker — scores hits by relevance, credibility, and freshness."""
...
from a2a_research.backend.agents.stubs.ranker.card import RANKER_CARD
...
__all__ = ["RankerExecutor", "build_http_app"]


class RankerExecutor(AgentExecutor):
    async def execute(self, context, event_queue):
        ...
        # Passthrough: return all URLs in order
        urls = []
        for h in hits:
...
def build_http_app():
    handler = DefaultRequestHandler(
        ...
        agent_card=RANKER_CARD,
    )
    return build_starlette_http_app(
        agent_card=RANKER_CARD, http_handler=handler
    )
