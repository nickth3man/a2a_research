...
from a2a_research.ui.formatting import format_claim_verdict, format_confidence
...
def _render_claim(claim: Claim) -> None:
    verdict_color_value = verdict_color(claim.verdict.value)
    verdict_bg_value = verdict_bg(claim.verdict.value)
    badge = format_claim_verdict(claim.verdict)
    confidence_label = format_confidence(claim.confidence)

    with me.box(
        style=me.Style(
            background=verdict_bg_value,
