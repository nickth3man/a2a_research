"""Pure UI session helpers (no Mesop imports) for testability and state
rules.

``has_results`` mirrors the UI's notion of "pipeline finished enough to
show output": either all agents are terminal (completed/failed) or a
non-empty ``final_report`` exists.
"""
...
def has_results(session: ResearchSession) -> bool:
    """True when the session has a complete-enough agent view or a
    final report."""
