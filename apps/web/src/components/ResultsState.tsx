...
function SummaryStrip({
  ...
  const avgConf = claims.length
    ? Math.round(
        (claims.reduce((a, c) => a + c.confidence, 0) / claims.length) * 100,
      ) + "%"
    : "—";
  return (
    <Paper
...
const verdictCfg: Record<
  Verdict,
  { color: string; bg: string; bd: string; glyph: string }
> = {
  SUPPORTED: {
    color: "var(--emerald)",
    bg: "var(--emerald-soft)",
    ...
    >
      <div
        style={{
          ...
          width: `${w * 100}%`,
          background: color,
          transition: "width 1.1s cubic-bezier(.2,.65,.2,1)",
        }}
      />
    </div>
  );
        ...
        {report.split("\n").map((line, i) => {
          ...
          if (line.startsWith("### "))
            return (
              ...
              >
                <em style={{ color: "var(--accent)", marginRight: 4 }}>§</em>{" "}
                {line.slice(4)}
              </h3>
            );
          if (line.startsWith("- "))
  ...
  return (
    ...
    >
      {warnings.length > 0 && (
        ...
        >
          <summary
            style={{ cursor: "pointer", fontWeight: 600, color: "#7c5f00" }}
          >
          ...
          >
            {warnings.map((d, i) => (
              <li key={i}>
                ...
                {d.role ? ` (${d.role})` : ""}:{" "}
                {d.root_cause || d.remediation || d.severity}
              </li>
            ))}
          </ul>
        </details>
              ...
              {sources.map((s, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "12px 0",
                    borderBottom:
                      i < sources.length - 1 ? "1px solid var(--rule)" : "none",
                  }}
                >
                  <span
                    className="serif"
