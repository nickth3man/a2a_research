"""Adversary — Devil's Advocate counter-evidence agent."""
...
from a2a_research.backend.agents.stubs.adversary.card import ADVERSARY_CARD
...
__all__ = ["AdversaryExecutor", "build_http_app"]


class AdversaryExecutor(AgentExecutor):
    async def execute(self, context, event_queue):
        ...
        # Passthrough: all claims pass as HOLDS
        challenge_results = []
        for c in claims:
...
def build_http_app():
    handler = DefaultRequestHandler(
        agent_executor=AdversaryExecutor(),
        task_store=InMemoryTaskStore(),
        agent_card=ADVERSARY_CARD,
    )
    return build_starlette_http_app(
        agent_card=ADVERSARY_CARD, http_handler=handler
    )
